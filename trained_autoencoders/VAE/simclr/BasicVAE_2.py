...
parser.add_argument('--char', default='feature_one', help='')
...
parser.add_argument('--outf',default='output_simclr_resnet18_features')
...
writer = SummaryWriter(logdir=os.path.join('BasicVAE_layerwise_TEST',opt.experiment,opt.ae_name))
shutil.copy('BasicVAE_2.py',os.path.join('BasicVAE_layerwise_TEST',opt.experiment,opt.ae_name))
...
for j in range(9):
    ...
    #         print(data)
        ...
        if epoch % opt.ckpt_epoch == 0:
            ...
            ckpt_path = os.path.join('BasicVAE_layerwise_TEST',opt.experiment,opt.ae_name,ckpt_name + ".pth")
...
#             print(i)
    ...
    l0 = open('./BasicVAE_layerwise_TEST/'+opt.experiment+'/'+opt.ae_name+'/confidence_layer_{}_in_{}_epoch_{}_train.txt'.format(j,opt.dataset,epoch), 'w')
...
#             print(i)
    ...
    rc_error_ind_total_np = rc_error_ind_total.detach().cpu().numpy()  
    ind_score = -rc_error_ind_total_np
    l1 = open('./BasicVAE_layerwise_TEST/'+opt.experiment+'/'+opt.ae_name+'/confidence_layer_{}_in_{}_epoch_{}.txt'.format(j,opt.dataset,epoch), 'w')
    for i in range(ind_score.shape[0]):
        l1.write("{}\n".format(ind_score[i]))
    l1.close()
...
#             print(i)
        ...
        l2 = open('./BasicVAE_layerwise_TEST/'+opt.experiment+'/'+opt.ae_name+'/confidence_layer_{}_out_{}_epoch_{}_model1.txt'.format(j,out_dataset[out_n],epoch), 'w')
