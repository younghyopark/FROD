...
#     x = F.normalize(x, dim=1)
    ...
    return F.cross_entropy(x_scale, targets.long().to(x_scale.device)), mean_norm
...
def train(args: DictConfig) -> None:
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        norm_meter = AverageMeter("norm_after_sigmoid")
        ...
        for x, y in train_bar:
            ...
            optimizer.zero_grad()
            feature, rep = model(x)
            loss,mean_norm = nt_xent(rep, args.temperature)
            loss.backward()
            optimizer.step()
            scheduler.step()

            loss_meter.update(loss.item(), x.size(0))
            norm_meter.update(torch.mean(mean_norm).item(), x.size(0))
            train_bar.set_description("Train epoch {}, SimCLR loss: {:.4f}, Norm: {:.4f}".format(epoch, loss_meter.avg, norm_meter.avg))
